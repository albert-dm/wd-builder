...
export const sendTarotMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { runChatTurn } = await import("./agent.server");
    const { requireAuthenticatedUser } = await import("./auth");
    const { getDb } = await import("./db.server");
    const { pixPayments } = await import("@webdrops/tarot-db");
    const { and, eq } = await import("drizzle-orm");
    ...
    // Call the AI agent with tool handlers
    ...
    // Save tool execution if any (for UI display)
    if (result.toolExecution) {
      const toolMessage = `Tool:${result.toolExecution.toolName} ->\n${result.toolExecution.rawOutput}`;
    ...
    await appendChatMessage(session.id, "Assistant", result.response);

    // Refresh session state after tool execution
    const updatedSession = await getOrCreateSession(authUser.id);
    const updatedHistory = await getChatHistory(session.id);
    const latestReading = await getLatestReading(session.id);
